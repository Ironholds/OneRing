tag_handler <- function(data){
  ...
  site[grepl(x = urls, pattern = "zero.", fixed = TRUE)] <- "Mobile web"
  site[grepl(x = urls, pattern = "m.w", fixed = TRUE)] <- "Mobile web"
  site[site == ""] <- "Desktop"
  ...
  mobile_site_version[site == "Mobile web" & !is_zero] <- "Production"
